...
class ToolsInstaller:
    ...
    def _check_and_install_tools(self, name, info):
        ...
        if exe_path.exists():
            try:
                ...
                if p.returncode == 0:
                    ...
                    if line1 == info["ver"]:
                        common.log.debug(f"{name} version verified: {line1}")
                        installed = True
                    else:
                        common.log.warning(f"{name} version mismatch: expected '{info['ver']}', got '{line1}'")
            except Exception as e:
                common.log.warning(f"Failed to check version for {name}: {e}")
        ...
        if not installed:
            ...
            common.log.info(f"Downloading {name} from {info['url']}...")
            ...
            try:
                ...
                if downloaded_hash.lower() != info["sha256"].lower():
                    os.remove(zip_fn)
                    common.log.error(f"SHA256 mismatch for {name}. Expected {info['sha256']}, got {downloaded_hash}")
                    raise ValueError(f"SHA256 mismatch for {name}. Downloaded file hash: {downloaded_hash}")
                common.log.info(f"SHA256 verified for {name}")

                with zipfile.ZipFile(zip_fn, 'r') as zf:
                    with zf.open(info["zip_path"]) as src, open(exe_path, "wb") as dst:
                        shutil.copyfileobj(src, dst)
                
                common.log.info(f"Installed {name} to {exe_path}")
            except Exception as e:
                common.log.error(f"Failed to install {name}: {e}")
                raise e
            finally:
                if zip_fn.exists():
                    os.remove(zip_fn)
        ...
        if "license_url" in info and "license_filename" in info:
            ...
            if not lic_path.exists():
                try:
                    common.log.info(f"Downloading license for {name}...")
                    urllib.request.urlretrieve(info["license_url"], lic_path)
                except Exception as e:
                    common.log.warning(f"Failed to download license for {name}: {e}")
