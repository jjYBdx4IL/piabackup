...
class SteamScanner:
    ...
    def scan_all(self):
        ...
        found = {}
        if base_steam:
            common.log.debug(f"Main Steam Path: {base_steam}")
            common.log.debug("-" * 40)
            
            libs = self.get_steam_libraries(base_steam)
            
            for lib in libs:
                common.log.debug(f"Scanning Library: {lib}")
                games = self.scan_games_in_library(lib)
                
                if not games:
                    common.log.debug("  No games found or empty library.")
                
                for game in games:
                    common.log.debug(f"  Found: {game['name']}")
                    common.log.debug(f"    -> {game['path']}")
                    found[game['name']] = game['path']
        else:
            common.log.debug("Could not locate Steam installation.")
        return found
