...
def toggle_auto_start(enable):
    if is_running_in_sandbox():
        try:
            ...
            common.log.error(f"Failed to open startup settings: {e}")
        return
    ...
    try:
        ...
        if enable:
            ...
            common.log.info("Autostart enabled")
        else:
            try:
                winreg.DeleteValue(key, common.APPNAME)
                common.log.info("Autostart disabled")
            except FileNotFoundError:
                pass
        winreg.CloseKey(key)
    except Exception as e:
        common.log.error(f"Failed to toggle autostart: {e}")
