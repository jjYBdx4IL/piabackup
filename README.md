...
* Auto-start on logon will fail after every udpate because the version is included in the installation path due to how MSIX is implemented. Just click that button in the settings dialog to update the task scheduler entry.
...
* Add restore support for entire backup dir.
...
* `sqlite3 sqlite.db ".schema" ; sqlite3 -table sqlite.db "select * from config; select * from status" ; sqlite3 -line sqlite.db "select * from backup_dirs;"`
